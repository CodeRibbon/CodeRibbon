...
module.exports = {
  ...
  activate(state) {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'code-ribbon:add': () => this.add()
    }));

    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'code-ribbon:remove': () => this.remove()
    }));
  ...
  // Dev command for adding patches
  add() {
    atom.config.set("code-ribbon.pane_count_calc.pane_count_horizontal_number", 
    atom.config.get("code-ribbon.pane_count_calc.pane_count_horizontal_number") + 1);
  },

  // Dev command for removing patches
  remove() {
    atom.config.set("code-ribbon.pane_count_calc.pane_count_horizontal_number", 
    atom.config.get("code-ribbon.pane_count_calc.pane_count_horizontal_number") - 1);
  }
