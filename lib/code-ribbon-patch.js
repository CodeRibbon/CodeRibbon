...
class CodeRibbonPatch extends Pane {
  ...
  initialize() {
    ...
    for (let i = 0; i < this.items.length; i++) {
      ...
      if (item.__proto__.constructor.name == "TextEditor") {
        ...
        replaceTextEditorComponent_didFocus(item);
    ...
    this.subscriptions.add(this.onDidAddItem(n_item => {
      ...
      if (
        ...
        // replaceTextEditorComponent_didFocus(n_item.item);
  ...
  move_secondary_item_elsewhere(focus_target = false) {
    ...
    if (pm) {
      ...
      if (focus_target) pm.smoothactivate();
      return;
    }
   ...
   */
  smoothactivate() {
    scrollPatchIntoView(this, () => this.activate());
  }
