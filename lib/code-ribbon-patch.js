...
class CodeRibbonPatch extends Pane {
  ...
  initialize() {
    ...
    if (this.items.length == 0) {
      ...
      // THIS WORKS FOR ONE EMPTY PATCH
      // atom.commands.dispatch(target, "application:about");

      atom.commands.dispatch(target, "fuzzy-finder:toggle-file-finder");
      atom.commands.onDidDispatch(function(e) {
        crdebug(e);
      })
      var obj = atom.commands.findCommands({target: "atom-pane.pane.cr-patch"});
      crdebug(obj); // fuzzy-finder not displayed here
    }
  }
