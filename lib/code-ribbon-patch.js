...
class CodeRibbonPatch extends Pane {
  ...
  initialize() {
    ...
    const inner_patch_scroll_handler = function (event) {
      // crdebug(event);
      if (
        (event.deltaX != 0)
        || (
        ...
        crdebug("Preventing Scroll event:", event);
        event.preventDefault();
      }
    }
    this.element.getElementsByClassName('item-views')[0].onwheel = inner_patch_scroll_handler;

    const tab_bar_onwheel_handler = function (event) {
    ...
    if (this.element.getElementsByClassName('tab-bar').length > 0) {
      try {
        this.element.getElementsByClassName('tab-bar')[0].onwheel = tab_bar_onwheel_handler;
      } catch (e) {
        crlogger.warn(this, e);
      }
    }
    else {
      // set up a listener to wait for the tab bar to be added
      const hook_tab_scrolling_mutation_config = {
        childList: true,
        subtree: false
      }
      const hook_tab_apply_scroll_listener = function (mutationsList, observer) {
        for (let mutation of mutationsList) {
          // crdebug(mutation);

          // put listener on if a tab bar got added, and disconnect
          if (
            mutation.addedNodes.length == 1 &&
            mutation.addedNodes[0].classList.contains('tab-bar')
          ) {
            crdebug("Adding onwheel to", mutation.addedNodes[0]);
            mutation.addedNodes[0].onwheel = tab_bar_onwheel_handler;
            observer.disconnect();
          }
        }
      }
      const hook_tab_mutation_observer = new MutationObserver(hook_tab_apply_scroll_listener);
      hook_tab_mutation_observer.observe(this.element, hook_tab_scrolling_mutation_config)
    }
  ...
  cr_focused() {
    this.element.classList.add('cr-focused');
    this.getParent().element.classList.add('cr-focused');

    this.element.style.zIndex = 10;
    this.element.style.width = (
      getHorizontalPatchesPerScreen() * 100.0
    ).toString() + "%";
  }

  cr_unfocused() {
    this.element.classList.remove('cr-focused');
    this.getParent().getElement().classList.remove('cr-focused');
