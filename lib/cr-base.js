...
function get_cr_panecontainer() {
  let cr_module = get_cr_module();
  return cr_module.main_CRM.cr_primary_container;
}

function get_crrc() {
  return get_cr_panecontainer().getRoot();
}
...
var contextMenuCommandsToDisable = [
  ...
  // "tabs:split-right",
  // "tabs:split-left",
  // stuff that expects multiple tabs:
...
var contextMenuCommandsToRename = [
  "pane:close",
]
...
function delayed_initialize() {
  ...
  // rename Pane to Patch
  atom.contextMenu.itemSets.map(menuItemSet => {
    menuItemSet.items.map((menuItem) => {
        if (
          contextMenuCommandsToRename.includes(menuItem.command)
        ) {
          debug.log(
            "Renaming context menu entry:",
            menuItemSet,
            menuItem
          );
          menuItem.label = menuItem.label.replace('Pane', 'Patch');
        }
      }
    );
  });
