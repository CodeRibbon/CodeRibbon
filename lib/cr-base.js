...
function replaceTextEditorComponent_didFocus(textEditorItem) {
  if (textEditorItem.__proto__.constructor.name != "TextEditor") {
    debug.error("replaceTextEditorComponent_didFocus got something that's not a TextEditor:", textEditorItem);
    debug.warn("replaceTextEditorComponent_didFocus will continue anyways, expect problems below!");
  }
  textEditorItem.component.didFocus = function() {
    ...
    console.warn("CodeRibbon Override TextEditorComponent didFocus: Focused hiddenInput with prevented scrolling.");
  };
  debug.warn("Replaced TextEditor's didFocus");
}
