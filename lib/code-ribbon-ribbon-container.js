...
class CodeRibbonRibbonContainer extends PaneAxis {
  ...
  cr_update() {
    ...
    this.child_ribbon_max_patch_count = 1;
    // crdebug("Finding child_ribbon_max_patch_count...")
    this.children.map(child => {
      // crdebug("Looking at ", child);
      if (child.children.length > this.child_ribbon_max_patch_count) {
        this.child_ribbon_max_patch_count = child.children.length;
      }
    });
    // TODO recalculation of real quota
    // to include reduction when too many blank patches
    this.patchQuota = this.child_ribbon_max_patch_count;
    ...
    var widthpercent = 100.0 * (
      this.child_ribbon_max_patch_count / atom.config.get("code-ribbon.pane_count_calc.pane_count_horizontal_number")
    );
