...
class CodeRibbonRibbonContainer extends PaneAxis {
  ...
  /**
   * used to keep track when two panes want to swap items,
   * but the drop event might fail, so we need a reference to the giving patch
   *
   * @param {CodeRibbonPatch} source_patch the patch that had it's item TAKEN
   */
  set_recent_patch_swap_source(source_patch) {
    this.recent_patch_swap_source = source_patch;
  }

  /**
   * used to get the patch that sent the moved item
   * @return {CodeRibbonPatch,undefined} the most recent patch that gave(move)
   */
  get_recent_patch_swap_source() {
    return this.recent_patch_swap_source;
  }
