...
class CodeRibbonRibbonContainer extends PaneAxis {
  ...
  initialize() {
    ...
    // remove context menu items that allow closing patches/panes:
    atom.contextMenu.itemSets = atom.contextMenu.itemSets.filter(
      function(val, index, arr) { // eslint-disable-line no-unused-vars
        return val.items.find(
          i => {
            return i.label == "Close Pane"
          }
        ) == undefined;
      }
    );
