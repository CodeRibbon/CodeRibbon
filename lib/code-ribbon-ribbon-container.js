...
class CodeRibbonRibbonContainer extends PaneAxis {
  ...
  element_transitionend_overview_mode(event) {
    crdebug("transition END", this, this.model, event);
    if (
   ...
   * NOTE `this` is the element!
   */
  element_transitionend_focus_mode(event) {
    crdebug("transition END", this, this.model, event);
    if (
      event.propertyName == "flex-grow" &&
      // this transistion event comes from the children
      this.model.children.find((child_ribbon_strip) => {
        event.srcElement == child_ribbon_strip.element
      })
    ) {
      return;
    }
    scrollPatchIntoView(this.model.previous_focus_active);
    // remove the listener after we're done
    this.removeEventListener(
      'transitionend',
      this.model.element_transitionend_focus_mode
    );
