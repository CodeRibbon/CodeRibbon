...
class CodeRibbonRibbonContainer extends PaneAxis {
  ...
  toggle_overview_mode() {
    ...
    if (this.overview_active == false) {
      ...
      this.horiz_transformpoint = (
        active_pane.getParent().getElement().offsetLeft
         - (
          active_pane.getParent().getElement().offsetWidth
        )
      );
  ...
  getHorizontalPatchesPerScreen() {
    if (
      ...
      if (
        ...
        crlogger.error("getHorizontalPatchesPerScreen() came up with unreasonable result:", n);
  ...
  cr_update() {
    ...
    while (
      ...
      crdebug("CRRC Adding strip to meet quota:", this.children.length, "of", this.stripQuota);
      this.cr_add_strip();
    }
  ...
  autoincrement_strip_quota() {
    ...
    const horiz_patches_per_screen = this.getHorizontalPatchesPerScreen();

    if (
      this.stripQuota <= horiz_patches_per_screen
    ) {
      this.stripQuota = horiz_patches_per_screen + 1;
      return true;
    }
    else if (
      ...
      crdebug("CRRC Incrementing stripQuota...");
      this.stripQuota++;
      return true;
    }
    else return false;
  ...
  autoremove_excess_empty_right_patch_column() {
    const horiz_target = this.getHorizontalPatchesPerScreen() + 1;
    if (this.children.length <= horiz_target) {
      crdebug("CRRC no excess patches to remove.");
      return false;
    }
    for (
      var i = this.children.length - 1;
      i >= this.children.length - horiz_target;
      i--
    ) {
