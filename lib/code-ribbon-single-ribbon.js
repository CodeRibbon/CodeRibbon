...
class CodeRibbonSingleRibbon extends PaneAxis {
  ...
  cr_update() {
    // can we reduce the quota?
    // check if the last two panes are empty:
    var min_empty_panes_for_quota_decrease = 2;
    if (
      this.children.length > this.getParent().patchQuota
      && this.children.length > min_empty_panes_for_quota_decrease
    ) {
      if (this.at_least_n_empty_right_patches(min_empty_panes_for_quota_decrease) == true) {
